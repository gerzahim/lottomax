      ...
      while($row= $obj_conexion->GetArrayInfo($resultTT)){
      	...
      	if ($row['incompleto']== 0 || $row['incompleto']== 1){
              ...
              if ($obj_generico->IsEmpty($yaesta)){
              	//echo "No ESTA";
                $result= ProcesoCupos($txt_terminal, $txt_monto, $row['id_sorteo'], $id_zodiacal, 1,$id_insert_taquilla);
              }              
	...
	if ( $numero_jugadoticket['total_registros']>0 ){
		...
		if( $numero_jugado['total_registros']>0 ){
			...
			if ($num_jug >0){
				...
				if( $obj_modelo->GuardarTicketTransaccional($txt_numero,$sorteo,$zodiacal,$id_tipo_jugada,$num_jug_nuevodisponible,$incompleto,$txt_monto,$taquilla,$id_insert_taquilla) ){
