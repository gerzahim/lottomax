...
if( $result= $obj_modelo->GetDetalleTicketByIdticket($id_ticket) ){
	...
	$i=0;
	for($i;$i<count($sorteosenticket);$i++){
		$j=0;
		for($j;$j<count($combinacionunica);$j++){
			
			//verificar si esta combinacion existe
			foreach ($ticket_completo as $tc){
				
				$combinacion_creada= $sorteosenticket[$i]."-".$combinacionunica[$j];
				if ($tc == $combinacion_creada){
					$combinacion_ticket[$sorteosenticket[$i]][]= $combinacionunica[$j];					
				}			
			}
		}
	}*/
	...
	for($i;$i<count($combinacionunica);$i++){
		...
		for($j;$j<count($sorteosenticket);$j++){
			...
			foreach ($ticket_completo as $tc){
				...
				$combinacion_creada= $sorteosenticket[$i]."x".$combinacionunica[$j];
				if ($tc == $combinacion_creada){
	...
	//echo count($sorteosenticket);
	//echo count($combinacionunica);
	echo "<pre>";
	print_r($combinacion_ticket);
	echo "</pre>";
	//print_r($combinacionunica);
}
