...
switch (ACCION){
	case 'mod':
		...
		if( is_numeric($_GET['id']) ){
            ...
            if ($row_datos["tipo_comision"]=='1'){
            	$obj_xtpl->assign('tipo_comision', '1');
                $obj_xtpl->assign('seleccion2', 'selected="selected"');
                $obj_xtpl->assign('nombre_tipo_comision', 'Ventas Brutas');
                $obj_xtpl->parse('main.contenido.formulario.tipo_comision');
                $obj_xtpl->assign('tipo_comision', '2');
                $obj_xtpl->assign('seleccion2', '');
                $obj_xtpl->assign('nombre_tipo_comision', 'Ganancia');
                $obj_xtpl->parse('main.contenido.formulario.tipo_comision');
			}
            else{
            	$obj_xtpl->assign('tipo_comision', '1');
	...
	case 'upd':
                ...
                if ($obj_generico->IsNumerico($_POST['op_tipo_comision'])){
                	$tipo_comision= $obj_generico->CleanText($_POST['op_tipo_comision']);
                }
		...
		if(!$obj_generico->IsEmpty($nombre_agencia) && !$obj_generico->IsEmpty($id_parametros)){
			// Modifica datos
			if( $obj_modelo->ActualizaDatosParametros($id_parametros, $id_agencia, $nombre_agencia, $tiempo_cierre_sorteos, $tiempo_anulacion_ticket, $tiempo_vigencia_ticket, $aprox_arriba, $aprox_abajo, $comision_agencia,$tipo_comision) ){
