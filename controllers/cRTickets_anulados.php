<?php
/**
 * Archivo del controlador para modulo de Reporte de Tcikets Anulados
 * @package cRTickets_anulados.php
...
// Vista asignada
$obj_xtpl->assign_file('contenido', $obj_config->GetVar('ruta_vista').'Rtickets_anulados'.$obj_config->GetVar('ext_vista'));
...
switch (ACCION){
    ...
    case 'listar_resultados':
        ...
        $i=0;
        if( $result= $obj_modelo->GetTicketsAnulados($fecha) ){
        ...
        // Configuracion de colores
         ...
         if( $result= $obj_modelo->GetTicketsAnulados($fecha) ){
            if ($obj_conexion->GetNumberRows($result)>0 ){
                ...
                $pdf->Cell(40,7,'Fecha Hora Anulacion',1,0,'C',true);
